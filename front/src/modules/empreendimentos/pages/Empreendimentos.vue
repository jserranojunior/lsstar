<template>
  <div>
    <header class="text-center text-primary font-weight-bold display">
      <h3 v-if="$route.params.tipoempreendimento == 'Á venda'">
        EMPREENDIMENTOS Á VENDA
      </h3>
      <h3 v-if="$route.params.tipoempreendimento == 'Vendida'">
        EMPREENDIMENTOS VENDIDOS
      </h3>
      <h3 v-if="$route.params.tipoempreendimento == 'Construção'">
          ...
          <div class="card-body">
            <table class="table table-striped table-condensed table-hover">
              ...
              <tbody>
                <tr
                  class="pointer"
                  v-for="(empreendimento, key, index) in Empreendimentos"
                  :key="empreendimento.id"
                  @click="edit(empreendimento.id)"
                >
                  <td>{{ index + 1 }}</td>
